...
namespace viewer.Controllers
{
    ...
    public class UpdatesController : Controller
    {
        ...
        public UpdatesController(IHubContext<GridEventsHub> gridEventsHubContext, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> Post()
        {
            using (var reader = new StreamReader(Request.Body, Encoding.UTF8))
            {
                ...
                // store in db
                var ProdConnectionString = _configuration.GetSection("Database:ProdConnectionString").Value;
                var TestConnectionString = _configuration.GetSection("Database:TestConnectionString").Value;
                string? ConnectionString = ProdConnectionString;
                string testPattern = "testtest ";
                if (jsonContent.IndexOf(testPattern, StringComparison.OrdinalIgnoreCase)  != -1)
                {
                    ConnectionString = TestConnectionString;
                    jsonContent = jsonContent.Replace(testPattern, string.Empty, StringComparison.OrdinalIgnoreCase);
                }
                StorageHelper storageHelper = new StorageHelper(ConnectionString);
                storageHelper.SaveSMSInbound(jsonContent, "SMS-INBOUND");
